...
class cart
{
    ...
    public function add($productId)
    {
        ...
        if ($check_cart) {
            ...
            $query_insert = "INSERT INTO cart VALUES(NULL,'$userId','$productId',1,'$size','$productName','$productPrice','$image' )";
    ...
    public function updateSize($productId, $size)
    {
        $userId = Session::get('userId');
        //Check qty product in db
        $query_insert = "UPDATE cart SET qty = $size WHERE productId = $productId AND userId = $userId";
        $insert_cart = $this->db->update($query_insert);
        if ($insert_cart) {
            return true;
        } else {
    ...
    {
        $userId = Session::get('userId');
        $query = "SELECT size FROM cart WHERE userId = '$userId' ";
        $mysqli_result = $this->db->select($query);
        if ($mysqli_result) {
            $result = mysqli_fetch_all($this->db->select($query), MYSQLI_ASSOC)[0];
            return $result;
